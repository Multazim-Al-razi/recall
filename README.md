...
A local-first subscription tracking webapp that monitors recurring charges, warns you before each renewal, and provides spending intelligence — all in your browser. Free and open: your data lives on your device and we never see it. Cloud-delivered reminders will be available via the Sync plan ($1.99/mo) once billing and reminder delivery ship — see [docs/BACKEND_ROADMAP.md](docs/BACKEND_ROADMAP.md). The cloud backend is migrating to **Supabase** (hosted Postgres, Auth, RLS, Realtime, Edge Functions).
...
- **Sync plan ($1.99/mo)** — Email & push renewal reminders that reach you with the tab closed. **In development** — see [BACKEND_ROADMAP.md](docs/BACKEND_ROADMAP.md). Gated by `FLAGS.syncPlan` in [`frontend/src/lib/featureFlags.ts`](frontend/src/lib/featureFlags.ts) so the marketing surface is never a promise the runtime can't keep.
...
| Backend | Express 5 + lowdb — **dev-only**, migrating to Supabase (Postgres + Auth + RLS + Realtime + Edge Functions). See [BACKEND_ROADMAP.md](docs/BACKEND_ROADMAP.md) |
| E2E | Playwright |
...
│   ├── vitest.config.ts
│   └── eslint.config.js
├── backend/                  # Express API (lowdb) — dev-only; Supabase for cloud (see BACKEND_ROADMAP.md)
│   └── src/
│       ├── index.ts          # Express server + production kill switch
│       ├── db.ts             # lowdb database (SupabaseAdapter coming)
│       ├── validate.ts       # input validation + sanitization
│       └── routes/           # accounts, subscriptions
├── supabase/                 # (to be created) Supabase migrations + Edge Functions
│   ├── config.toml           # Supabase CLI config
│   ├── migrations/           # 001_init.sql, 002_rls.sql, 003_reminders.sql
│   └── functions/            # send-reminders, daily-digest
...
│   └── BACKEND_ROADMAP.md    # Phase 7 — Sync plan + cloud-ready backend
...
- [BACKEND_ROADMAP.md](docs/BACKEND_ROADMAP.md) — Phase 7: Sync plan + cloud-ready backend
