...
/** Convenience hook: does the active plan unlock a feature gated at `tier`? */
export function useHasFeature(requiredTier: PlanTier): boolean {
  const plan = useAccountStore((s) => s.plan);
  return planIncludes(plan, requiredTier);
}
